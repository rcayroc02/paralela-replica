...
def visualize_grid(grid: np.ndarray, path: Optional[List[Tuple[int, int]]] = None) -> str:
    ...
    print(f"Dividiendo el grid en parches de {patch_size}x{patch_size}...")
    patches = divide_grid_into_patches(grid, width, height, patch_size)

    print(f"Se generaron {len(patches)} parches.")
    for idx, patch in enumerate(patches):
        print(f"Parche {idx + 1} tiene dimensiones: {patch.shape}")
...
def exec_1():
    ...
    print("Generando grid para GPU 0...")
    grid = generate_random_obstacles(width, height, start, goal, obstacle_density=0.0)
    
    cuda.init()
    device_1 = cuda.Device(0)
    context_1 = device_1.make_context()
    
    total_elapsed_time = 0.0
    total_expanded_states = 0
    
    for i in range(2):
        print(f"Iteración {i + 1} en GPU 0...")
        context_1.push()
        astar_1 = AStarCUDA(grid, max_gpu_memory=max_gpu_memory)
        
        start_time = time.time()
        path_1 = astar_1.find_path(start, goal)
        end_time = time.time()
        
        elapsed_time_1 = end_time - start_time
        expanded_states_1 = astar_1.get_expanded_states()
        
        total_elapsed_time += elapsed_time_1
        total_expanded_states += expanded_states_1
        
        context_1.pop()
    
    total_expansion_rate = (total_expanded_states / total_elapsed_time) / 1000
    
    print(f"GPU 0 - Tiempo total: {total_elapsed_time:.3f} segundos")
    print(f"GPU 0 - Estados expandidos: {total_expanded_states:,}")
    print(f"GPU 0 - Tasa de expansión: {total_expansion_rate:.2f} nodos/ms")
...
def exec_2():
    ...
    print("Generando grid para GPU 1...")
    grid = generate_random_obstacles(width, height, start, goal, obstacle_density=0.0)
    
    cuda.init()
    device_2 = cuda.Device(1)
    context_2 = device_2.make_context()
    
    total_elapsed_time = 0.0
    total_expanded_states = 0
    
    for i in range(2):
        print(f"Iteración {i + 1} en GPU 1...")
        context_2.push()
        astar_2 = AStarCUDA(grid, max_gpu_memory=max_gpu_memory)
        
        start_time = time.time()
        path_2 = astar_2.find_path(start, goal)
        end_time = time.time()
        
        elapsed_time_2 = end_time - start_time
        expanded_states_2 = astar_2.get_expanded_states()
        
        total_elapsed_time += elapsed_time_2
        total_expanded_states += expanded_states_2
        
        context_2.pop()
    
    total_expansion_rate = (total_expanded_states / total_elapsed_time) / 1000
    
    print(f"GPU 1 - Tiempo total: {total_elapsed_time:.3f} segundos")
    print(f"GPU 1 - Estados expandidos: {total_expanded_states:,}")
    print(f"GPU 1 - Tasa de expansión: {total_expansion_rate:.2f} nodos/ms")

def main():
    process1 = multiprocessing.Process(target=exec_1)
    process2 = multiprocessing.Process(target=exec_2)
